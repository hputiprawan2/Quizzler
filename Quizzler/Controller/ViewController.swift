...
class ViewController: UIViewController {
    ...
    @objc private func updateUI() {
        questionLabel.text = quizes[questionNumber].questionText
        trueButton.backgroundColor = UIColor.clear
        falseButton.backgroundColor = UIColor.clear
        progressBar.progress = Float(questionNumber + 1) / Float(quizes.count) // questionNumber + 1 to tell the progress from the start
    }
}
